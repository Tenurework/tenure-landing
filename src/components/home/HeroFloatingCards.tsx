...
export function HeroFloatingCards() {
  ...
  return (
    <div aria-hidden className="pointer-events-none absolute inset-0 z-20 hidden xl:block">
      ...
      <motion.div
        className="absolute -left-8 top-[10%] w-[14.5rem] -rotate-2 rounded-2xl border border-coral/25 bg-cloud/95 p-3 shadow-[0_28px_54px_-18px_rgba(12,30,51,0.4)] backdrop-blur"
      ...
      >
        ...
        <motion.div className="transform-gpu will-change-transform" {...float(-7, 6.5, 0)}>
          ...
          <p className="mt-1.5 text-[0.7rem] leading-snug text-ink-soft">
            Schlegel 207 double-booked 5:00 to 6:30p. <span className="font-medium text-[#b23a1f]">Auto-flagged.</span>
          </p>
        </motion.div>
      </motion.div>
      ...
      <motion.div
        className="absolute -left-10 bottom-[12%] w-[15rem] rotate-[1.5deg] rounded-2xl border border-grove/25 bg-cloud/95 p-3 shadow-[0_28px_54px_-18px_rgba(20,99,63,0.36)] backdrop-blur"
        initial={{ opacity: 0, y: 14 }}
        animate={{ opacity: 1, y: 0 }}
